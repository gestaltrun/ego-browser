...
declare function require(id: string): any
		...
		// The Sidebar tab and standalone preview share one interactive React view.
		...
		// The optional Sidebar provider is observed in a child injection scope.
		const inject = ['slots', 'locale', 'connection']

		// ── Settings card: locale ─────────────────────────────────────────
		var SETTINGS_NS = 'ego-browser'
		var en = {
			watchTitle: 'Agent browser', watchToggle: 'Show or hide the interactive browser preview',
			interactionReady: 'Interactive', interactionUnavailable: 'Input unavailable', interactionRetry: 'Input unavailable; this action was not sent. Try again.', manualInputHint: 'Stop the current agent turn before interacting. Send a new instruction when finished.',
			title: 'ego-browser',
		...
		var zh = {
			watchTitle: 'Agent 浏览器', watchToggle: '显示或隐藏交互浏览器预览',
			interactionReady: '可交互', interactionUnavailable: '输入不可用', interactionRetry: '输入不可用，本次操作未送达，请重试。', manualInputHint: '手动操作前请停止当前 Agent 回合，完成后再发送继续指令。',
			title: 'ego-browser',
...
`;
		...
		function mountFloatingWatch(ctx) {
			...
			var render = function () {
				...
				root.render(React.createElement(EgoBrowserTab, { ctx: ctx, visible: visible }))
			}
			button.onclick = function () { visible = !visible; render() }
		...
		// Shared by the Sidebar outlet and the standalone floating container.
		var TAB_CSS = `
.dsh-ego-side-root {
...
`
		...
		function EgoBrowserTab(props) {
			...
			var ctx = props.ctx
			var t = ctx.locale.bind(SETTINGS_NS)
			React.useSyncExternalStore(React.useCallback(function (notify) { return ctx.locale.subscribe(notify) }, [ctx]), React.useCallback(function () { return ctx.locale.getSnapshot().revision }, [ctx]))
			...
			if (!currentSpace) {
				...
				body = h('div', { className: 'dsh-ego-side-body' },
					h('div', { className: 'dsh-ego-side-liveview' },
						h('div', { className: 'dsh-ego-side-livebadge', title: t('manualInputHint') },
							h('span', { className: 'dsh-ego-side-state-dot' + (state.pinned ? ' pin' : state.busy ? ' busy' : '') }),
							h('span', { style: { flex: 1 } }, (state.backend === 'ffmpeg' ? 'FFmpeg · H.264' : 'CDP') + ' · ' + (state.streamState === 'failed' ? (state.streamMessage || '失败') : state.streamState) + (state.currentTargetId ? (state.wiringStale ? ' · ' + t('interactionUnavailable') : ' · ' + t('interactionReady')) : '')),
							state.pinned
								? h('button', {
		...
		function mountSidebarTab(ctx, betterSidebar) {
			...
			var disposeTab = betterSidebar.registerTab({
				...
				title: function () { return ctx.locale.bind(SETTINGS_NS)('watchTitle') },
				order: 70,
				single: true,
				component: function (props) {
					return React.createElement(EgoBrowserTab, { ctx: ctx, visible: props.visible })
				},
			})

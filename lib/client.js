window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		let _microsoft_fetch_event_source = require("@microsoft/fetch-event-source");
		...
		var EgoEventSource = class {
			...
			onmessage = null;
			constructor(url) {
				(0, _microsoft_fetch_event_source.fetchEventSource)(url, {
...
`;
		...
		function apply(ctx) {
			...
			try {
				betterSidebarService = ctx.get("betterSidebar");
			} catch (e) {

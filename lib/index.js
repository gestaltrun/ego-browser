...
function egoRequestRejection(ctx, request) {
	if ((ctx.get?.("desktopPrivateHttp"))?.isTrusted(request)) return void 0;
	const connection = ctx.get?.("connection");
	return connection === void 0 ? 401 : connection.requestRejection(request);
}
//#endregion
...
function managedRuntimeRoot(env = process.env) {
...
function managedStateDirectory(env = process.env) {
...
function managedRuntimeEnvironment(base = process.env) {
	const env = { ...base };
...
function makeEnsureWorker(ctx, cfg, ffmpegManager) {
	...
	return () => pending ??= startWorker().finally(() => {
		pending = void 0;
	});
}
/**
	...
	ctx.effect?.(() => {
		...
		return webServer.register({
			...
			handler: async (reqRaw, resRaw) => {
				...
				const rejection = egoRequestRejection(ctx, req);
				if (rejection !== void 0) {
					writeJson(res, rejection, envelopeError("unauthorized", "Host authentication required"));
					return;
				}
				if (req.method !== "POST") {
...
function findChromeBinary(env = process.env) {
	if (env.EGO_LINUX_CHROME) return env.EGO_LINUX_CHROME;
	if (IS_WIN) {
		for (const p of windowsChromeCandidates()) try {
			if (existsSync(p)) return p;
		} catch {}
		const exts = (env.PATHEXT ?? ".EXE;.CMD;.BAT;.COM").split(";").filter(Boolean).map((e) => e.startsWith(".") ? e.toLowerCase() : `.${e.toLowerCase()}`);
		const dirs = (env.PATH ?? "").split(";").map((d) => d.replace(/^"|"$/g, "")).filter(Boolean);
	...
	else for (const dir of (env.PATH ?? "").split(":")) {
		if (!dir) continue;
		const p = `${dir}/${name}`;
		try {
			if (existsSync(p)) return p;
...
function resolveEgoEnv(cfg, { platform = process.platform, baseEnv = process.env } = {}) {
	const env = managedRuntimeEnvironment(baseEnv);
	...
	reg(defineTool({
		...
		execute: async () => withEgoLock(async () => {
			try {
				...
				const stateDir = managedStateDirectory(e) || (isWin ? (e.LOCALAPPDATA || `${home}\\AppData\\Local`) + "\\ego-lite-linux" : `${e.XDG_STATE_HOME || `${home}/.local/state`}/ego-lite-linux`);
				let port = null;
				try {
...
function registerActionTools(ctx, cfg, reg) {
	...
	reg(t({
		name: "ego_space_open",
		description: "Open (or reuse) an ego-lite task space — an isolated browsing context within the plugin profile. It becomes the active space for later ego_* calls that omit `space`.",
		parameters: { name: {
			type: "string",
			required: true,
...
function registerHelpAndDoctor(ctx, cfg, reg) {
	...
	reg(defineTool({
		...
		execute: async () => {
			...
			lines.push(`browser available: ${chrome !== void 0 && existsSync(chrome)}`);
			const configured = cfg.chromePath;
			if (configured) lines.push(`browser binary: ${configured} (from settings)`);
			else lines.push(`browser binary: ${chrome || "(none found — install Chrome/Edge or set chromePath in plugin settings)"}`);
